...
class drawResultPlot2:
    ...
    def drawResultPlot(self, conn, table13, table15, table22):
        ...
        ax.set_title(r"$\bullet$ Position 20", loc="left")
        #        title=r'The result of SF1_OFF_SF2_ON_Tests with coil version 4 position 25')
        #        ax.text(left,top)
        tick_spacing = 5
        ...
        plt.plot(
            ...
            N_ucn_norm_time_offset_0,
            cs_label[0],
            label="Guiding coil with current on",
            linestyle="-",
            linewidth=2.0,
        )
        plt.plot(
            ...
            N_ucn_norm_time_offset_0_1,
            cs_label[1],
            label="Guiding coil with current off",
            linestyle="-",
            linewidth=2.0,
        )

        # plt.plot(sf2_frequency,N_ucn_norm_time_offset_10,cs_label[1],label =
        # 'time_offset 10s',linestyle='-',linewidth=2.0)
        # plt.plot(sf2_frequency,N_ucn_norm_time_offset_20,cs_label[2],label =
        # 'time_offset 20s',linestyle='-',linewidth=2.0)
        plt.legend()
        fig.savefig(
            A.DB_FILE_PATH_3
            + "\\pictures\\test_sf1_off_sf2_on_position_20.png",
            dpi=300,
            bbox_inches="tight",
        )
        plt.show()
